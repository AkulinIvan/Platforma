...
from users.special_permissions import SpecialPermissions
...
class LoginFormView(FormView):
    ...
    success_url = reverse_lazy('list_of_request:applications')
...
def access_sections(user):
    ...
    sections = ['Мои заявки','Открытые','Закрытые']
    applications = Articles.objects.filter(user=user)
    if applications:
        if applications.filter(status='ASSIGNED').exists():
            applications.filter(status='ASSIGNED')
        if applications.filter(status='COMPLETED').exists():
            applications.filter(status='COMPLETED')
